...
export default function ProgramDayEditorScreen() {
  ...
  useEffect(() => {
    if (!day) return;
    const hasEstablishedValues = Boolean(day.name && day.label && day.tag && day.session);
    ...
    return (
      <View testID="program-day-editor-screen" style={[styles.container, styles.center]}>
        <View style={styles.notFoundCard}>
          ...
          <Text style={styles.notFoundText}>This program day no longer exists.</Text>
        </View>
      </View>
    );
  }
  ...
  const handleDuplicateSlot = (slotId: string) => {
    const duplicatedId = duplicateSlot(slotId);
    if (duplicatedId) {
      ...
      <View style={styles.header}>
        ...
        <View style={styles.headerCopy}>
          <View style={styles.headerEyebrowRow}>
            <Text style={styles.headerEyebrow}>Active Program Day</Text>
            <ToneBadge label={day.rest ? 'Rest Day' : 'Training Day'} tone={day.rest ? 'rest' : 'live'} />
          </View>
          <Text style={styles.headerTitle}>{day.session}</Text>
              <Text style={styles.headerSubtitle}>{day.label} | {day.name} | {day.tag}</Text>
        </View>
        {!day.rest && (
      ...
      >
        <View style={styles.summaryCard}>
          <SectionHeader
            ...
            title={day.session}
            detail={`${day.label} | ${day.name} | ${day.tag}`}
            action={
              <TouchableOpacity
          ...
          <View style={styles.summaryMetaRow}>
            <View style={styles.summaryMetaCard}>
              <Text style={styles.summaryMetaLabel}>State</Text>
          ...
          {expandedSettings && (
            <View style={styles.settingsPanel}>
              ...
              <TextInput style={styles.input} value={day.name} onChangeText={name => updateDay(day.id, { name })} placeholder="Day name" placeholderTextColor={Colors.text3} />
              <View style={styles.twoCol}>
                <View style={{ flex: 1 }}>
                  <Text style={styles.fieldLabel}>Short Label</Text>
                  <TextInput style={styles.input} value={day.label} onChangeText={label => updateDay(day.id, { label: label.toUpperCase().slice(0, 6) })} placeholder="MON" placeholderTextColor={Colors.text3} autoCapitalize="characters" />
                </View>
                <View style={{ flex: 1 }}>
                  <Text style={styles.fieldLabel}>Tag</Text>
                  <TextInput style={styles.input} value={day.tag} onChangeText={tag => updateDay(day.id, { tag })} placeholder="Tag line" placeholderTextColor={Colors.text3} />
                </View>
              </View>
              <Text style={styles.fieldLabel}>Session Title</Text>
              <TextInput style={styles.input} value={day.session} onChangeText={session => updateDay(day.id, { session })} placeholder="Session title" placeholderTextColor={Colors.text3} />
              <Text style={styles.fieldLabel}>Protocol Notes</Text>
              <TextInput style={[styles.input, styles.textarea]} value={day.protocol ?? ''} onChangeText={protocol => updateDay(day.id, { protocol })} placeholder="Add coaching notes for this day" placeholderTextColor={Colors.text3} multiline textAlignVertical="top" />
              <View style={styles.toggleCard}>
                ...
                <Switch value={day.rest} onValueChange={rest => updateDay(day.id, { rest })} trackColor={{ false: Colors.border2, true: Colors.orange }} thumbColor="#fff" />
              </View>
              <Text style={styles.fieldLabel}>Accent Color</Text>
              <View style={styles.colorRow}>
                {COLOR_OPTIONS.map(color => (
                  <TouchableOpacity key={color} style={[styles.swatch, { backgroundColor: color }, day.color === color && styles.swatchActive]} onPress={() => updateDay(day.id, { color })} activeOpacity={0.85}>
                      ...
                      <View style={styles.slotSummaryHeader}>
                        ...
                        <View style={styles.slotSummaryActions}>
                          <View style={styles.reorderStack}>
                            <TouchableOpacity style={[styles.reorderButton, isFirst && styles.reorderButtonDisabled]} onPress={() => moveSlot(slot.id, 'up')} disabled={isFirst} activeOpacity={0.85}>
                              <Feather name="arrow-up" size={14} color={isFirst ? Colors.text3 : Colors.text2} />
                            </TouchableOpacity>
                            <TouchableOpacity style={[styles.reorderButton, isLast && styles.reorderButtonDisabled]} onPress={() => moveSlot(slot.id, 'down')} disabled={isLast} activeOpacity={0.85}>
                      ...
                      <View style={styles.slotExpanded}>
                        <View style={styles.editorSection}>
                          ...
                          <View style={styles.twoCol}>
                            ...
                            {isWger ? (
                              <TouchableOpacity style={styles.secondarySplitAction} onPress={() => clearCatalogAssignment(slot.id)} activeOpacity={0.85}>
                          ...
                          {!isWger && (
                            <>
                              ...
                              <TextInput style={styles.input} value={slot.exerciseName} onChangeText={exerciseName => updateSlot(slot.id, { exerciseName })} placeholder="Exercise name" placeholderTextColor={Colors.text3} />
                            </>
                          )}
                        </View>
                        ...
                        <View style={styles.editorSection}>
                          ...
                          <View style={styles.stepperGrid}>
                            <LargeStepper label="Sets" value={slot.sets} onDec={() => updateSlot(slot.id, { sets: Math.max(1, slot.sets - 1) })} onInc={() => updateSlot(slot.id, { sets: slot.sets + 1 })} />
                            <LargeStepper label="Rest" value={`${slot.restSeconds}s`} onDec={() => updateSlot(slot.id, { restSeconds: Math.max(0, slot.restSeconds - 15) })} onInc={() => updateSlot(slot.id, { restSeconds: slot.restSeconds + 15 })} />
                            <LargeStepper label="Rep Min" value={slot.repRange[0]} onDec={() => updateSlot(slot.id, { repRange: [Math.max(1, slot.repRange[0] - 1), slot.repRange[1]] })} onInc={() => updateSlot(slot.id, { repRange: [slot.repRange[0] + 1, Math.max(slot.repRange[1], slot.repRange[0] + 1)] })} />
                            <LargeStepper label="Rep Max" value={slot.repRange[1]} onDec={() => updateSlot(slot.id, { repRange: [slot.repRange[0], Math.max(slot.repRange[0], slot.repRange[1] - 1)] })} onInc={() => updateSlot(slot.id, { repRange: [slot.repRange[0], slot.repRange[1] + 1] })} />
                          </View>
                        </View>
                        ...
                        <View style={styles.editorSection}>
                          ...
                          <View style={styles.stepperGrid}>
                            <LargeStepper label="Load Step" value={`${slot.loadStep} lbs`} onDec={() => updateSlot(slot.id, { loadStep: Math.max(1, slot.loadStep - 1) })} onInc={() => updateSlot(slot.id, { loadStep: slot.loadStep + 1 })} />
                            <LargeStepper label="Stall After" value={`${slot.stallThreshold}x`} onDec={() => updateSlot(slot.id, { stallThreshold: Math.max(2, slot.stallThreshold - 1) })} onInc={() => updateSlot(slot.id, { stallThreshold: slot.stallThreshold + 1 })} />
                            <LargeStepper label="Min Sessions" value={slot.minSessionsBeforeStall} onDec={() => updateSlot(slot.id, { minSessionsBeforeStall: Math.max(2, slot.minSessionsBeforeStall - 1) })} onInc={() => updateSlot(slot.id, { minSessionsBeforeStall: slot.minSessionsBeforeStall + 1 })} />
                            <LargeStepper label="Deload %" value={`${Math.round(slot.deloadFactor * 100)}%`} onDec={() => updateSlot(slot.id, { deloadFactor: Math.max(0.7, Math.round((slot.deloadFactor - 0.05) * 100) / 100) })} onInc={() => updateSlot(slot.id, { deloadFactor: Math.min(0.95, Math.round((slot.deloadFactor + 0.05) * 100) / 100) })} />
                          </View>
                        </View>
                        ...
                        <View style={styles.editorSection}>
                          ...
                          <View style={styles.segmentedControl}>
                            <TouchableOpacity style={[styles.segment, !slot.failure && styles.segmentActive]} onPress={() => updateSlot(slot.id, { failure: false })} activeOpacity={0.85}>
                              <Text style={[styles.segmentText, !slot.failure && styles.segmentTextActive]}>1-2 RIR</Text>
                            </TouchableOpacity>
                            <TouchableOpacity style={[styles.segment, slot.failure && styles.segmentActive]} onPress={() => updateSlot(slot.id, { failure: true })} activeOpacity={0.85}>
                          ...
                          <View style={styles.wrap}>
                            <TouchableOpacity style={[styles.chip, slot.strengthSignalKey === null && styles.chipActive]} onPress={() => updateSlot(slot.id, { strengthSignalKey: null })} activeOpacity={0.85}>
                              <Text style={[styles.chipText, slot.strengthSignalKey === null && styles.chipTextActive]}>None</Text>
                            </TouchableOpacity>
                            {STRENGTH_LIFT_ORDER.map(signalKey => (
                              <TouchableOpacity key={signalKey} style={[styles.chip, slot.strengthSignalKey === signalKey && styles.chipActive]} onPress={() => updateSlot(slot.id, { strengthSignalKey: signalKey as StrengthLiftKey })} activeOpacity={0.85}>
                        ...
                        <View style={styles.editorSection}>
                          ...
                          <TextInput style={[styles.input, styles.textarea]} value={slot.note} onChangeText={note => updateSlot(slot.id, { note })} placeholder="Programming notes for this exercise" placeholderTextColor={Colors.text3} multiline textAlignVertical="top" />
                ...
                action={
                  activeSlot?.exerciseSource === 'wger' ? (
                    <TouchableOpacity
                      style={styles.inlineAction}
                      onPress={() => {
                        if (activeSlot) clearCatalogAssignment(activeSlot.id);
                        setPickerSlotId(null);
                      }}
                      activeOpacity={0.85}
                    >
                ...
                <View style={styles.resultList}>
                  {results.map(exercise => (
                    <TouchableOpacity
                      ...
                      onPress={() => {
                        if (activeSlot) assignCatalogExercise(activeSlot.id, exercise);
                        setPickerSlotId(null);
                      }}
                      activeOpacity={0.88}
                    >
